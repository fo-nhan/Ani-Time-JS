...
export const random = <T = any>(
  options: RandomOptions<T> = {}
  ...
  if (min !== undefined && max !== undefined) {
    ...
    if (filteredRange.length === 0) {
      throw new Error(
        "Không có giá trị nào trong phạm vi sau khi áp dụng các điều kiện loại trừ"
      );
    }
  ...
  if (pool.length > 0) {
    ...
    if (filteredPool.length === 0) {
      throw new Error(
        "Không có ký tự nào hợp lệ để random sau khi áp dụng các điều kiện loại trừ"
      );
    }
  ...
  if (data.length > 0) {
    ...
    if (filteredData.length === 0) {
      throw new Error(
        "Không có phần tử nào trong data sau khi áp dụng các điều kiện loại trừ"
      );
    }
  ...
  throw new Error(
    "Không có dữ liệu hợp lệ để random. Vui lòng cung cấp tập ký tự, phạm vi số hoặc mảng dữ liệu."
  );
  ...
  function generateUUID(
    options: { dashes?: boolean; version?: number } = {}
  ): string {
  ...
  function generateColor(
    ...
    switch (format) {
      ...
      default:
        ...
        if (alpha) {
          const alphaHex = Math.floor(parseFloat(a as string) * 255)
            .toString(16)
            .padStart(2, "0");
          hexColor += alphaHex;
        }
